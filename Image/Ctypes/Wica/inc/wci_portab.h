...
// Current debug level
#define DPRINTF_LEVEL	    0x80000103 // DPRINTF_ERROR, DPRINTF_WARNING
...
#ifndef ARCH_IS_GENERIC
#define ARCH_IS_GENERIC
#endif

#define wci_malloc AllocatePool
#define wci_free FreePool
#define wci_memcpy CopyMem
#define wci_memmove CopyMem
#define wci_memset(buff, val, size) SetMem(buff, size, val)
#define wci_strncmp AsciiStrnCmp

#define assert( xx_exp ) ASSERT( xx_exp )
...
#define wci_malloc malloc
#define wci_free free
#define wci_memcpy memcpy
#define wci_memmove memmove
#define wci_memset memset
#define wci_strncmp strncmp
 ...
 *---------------------------------------------------------------------------*/
 ...
 /* Needed for all debug calls */
#include <Library/DebugLib.h>
#include <Library/PrintLib.h>
#include <Library/SerialPortLib.h>

#    if !defined(MDEPKG_NDEBUG) && (DPRINTF_LEVEL > 0)

static __inline void DPRINTF(int level, const char* format, ...)
{
    VA_LIST args;
    CHAR8 buf[DPRINTF_MSG_SIZE + 1];
    VA_START(args, format);
    AsciiVSPrint(buf, DPRINTF_MSG_SIZE, format, args);
...
#   if defined(_DEBUG) && (DPRINTF_LEVEL > 0)
        if (DPRINTF_LEVEL & level) {
            ...
            vsprintf_s(buf, DPRINTF_MSG_SIZE, fmt, args);
#else
