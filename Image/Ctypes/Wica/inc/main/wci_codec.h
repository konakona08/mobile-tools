...
HISTORY:     2004.08.19   v1.00
			 ...
			 2024.02.05   v1.19 Added:   bootanimation for android!
...
typedef enum tagBOOTANIM_PARTTYPE
{
  BOOTANIM_PARTTYPE_PLAY_INTERRUPT   = 0 		//p -- this part will play unless interrupted by the end of the boot
, BOOTANIM_PARTTYPE_PLAY_COMPLETION  = 1		//c -- this part will play to completion, no matter what
, BOOTANIM_PARTTYPE_PLAY_FADE        = 2		//f -- same as p but in addition the specified number of frames is being faded out while continue playing.
												//     Only the first interrupted f part is faded out, other subsequent f parts are skipped

} BOOTANIM_PARTTYPE;

typedef struct tagBOOTANIM_PART
{
  BOOTANIM_PARTTYPE ePartType;				   //! Part type
  int				iTimes;					   //! How many times to play the animation, 0 = Forever
  int               iFramesToDelay;			   //! Number of FRAMES to delay after this part ends
  int				iFramesToFade;			   //! number of frames to fade out when interrupted where 0 means immediately which makes f ... 0 behave like p and doesn't count it as a fading part
} BOOTANIM_PART;

typedef struct tagBOOTANIM
{ 
  int       		iWidth;                    //! Image width [pixel]
  int       		iHeight;                   //! Image height [pixel]
  int       		iFrameRate;                //! Frame rate [FPS] 
  int				iParts;					   //! Desc.txt parts
  BOOTANIM_PART*	prPartInfo;				   //! Part info
} BOOTANIM;
...
ERROR_CODE wci_get_bootanim
(
  STREAM *          IN OUT prStream
, BOOTANIM *        IN OUT prBootAnim
);
