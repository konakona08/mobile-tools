  ...
  while ( nOutOffset < iDataLength )
  {
    if ( prContext->prRBC->nCount == 0 || prContext->prRBC->nCount == -1 )
    {
      ...
      if (nBytesToRead > 0)
      {
        ...
        wci_memcpy(&abtData, &prContext->pbtData[prContext->nPos], nBytesToRead);
...
)
{
  if ( iDataLength > 0 )
  {
    ...
    case QSP_CMP_RAW:
      wci_memcpy( pbtData, prContext->pbtData, iDataLength );
...
{
  wci_memset( prContext, 0, sizeof(QSP_COMPONENT_CONTEXT) );
