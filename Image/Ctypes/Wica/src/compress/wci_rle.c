...
// This function decompress data to the bit buffer using  RL decoding
  ...
  while ( iIndex < iCompressedDataLength )
  {
    ...
    else
    {
      ...
      wci_memset( &pbtData[iOutLength], btChar, iCount );
      iOutLength += iCount;
    }
...
{
  wci_memset( prC, 0, sizeof(RLE_BIT_CONTEXT) );
...
)
{
  ...
  if ( prC->nInLength > 0 )
  {
    ...
    wci_memmove( prC->abtInData, &prC->abtInData[prC->nPos], prC->nInLength );
  }

  prC->nPos = 0;
  wci_memcpy( &prC->abtInData[prC->nInLength], pbtInData, nInLength );
...
//  if( prC->btBit == RLE_MAX_BIT_REPEATION )
 if( prC->nCount < 0)
 {
    ...
    if ( prC->nCount > 0 )
    {
      ...
      if ( pbtOutData != NULL )
        wci_memset( &pbtOutData[nOut], prC->btBit, nCount );
      nOut += nCount;
      prC->nCount -= nCount;
