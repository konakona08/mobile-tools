...
)
{
  ...
  wci_memset( prComponent, 0, sizeof(IMAGE_COMPONENT) );
...
)
{
  ...
  wci_memset( pbtData, 0, wci_get_square(prComponent->rEdgedSize) );
}
...
)
{
  ...
  wci_memcpy( pbtDst, pbtSrc, wci_get_square(prSrc->rEdgedSize) );
}
...
)
{
  ...
  if ( rRectSize.cx < w )
  {
    ...
    for ( y=0; y<rRectSize.cy; ++y )
    {
      ...
      wci_memset( &pbtLine[rRectSize.cx], pbtLine[rRectSize.cx-1], iDiff );
    }
  }
  ...
  if ( rRectSize.cy < h )
  {
    ...
    {
      wci_memcpy( pbtDstLine, pbtSrcLine, w );
      pbtDstLine += w;
    }
  }
}
...
)
{
  ...
  wci_memset( prComponent, 0, sizeof(DIFF_IMAGE_COMPONENT) );
