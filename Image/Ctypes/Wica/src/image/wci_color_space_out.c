...
)
{
  ...
  for (j = 0; j < nEvenH; j+=(2))
  {
    ...
    for (i = 0; i < nEvenW; i+=(2))
    {
      ...
      YUV_TO_BGRA_DOUBLE( piDst, rgb_y, rgb_y1, b_u0, g_uv0, r_v0, 255, 255); //0,0
   
      rgb_y = *(int*)&g_RGB_Y_tab[ pbtY[ nStrideY ] ];
      rgb_y1 = *(int*)&g_RGB_Y_tab[ pbtY[ nStrideY + 1 ] ];     
      YUV_TO_BGRA_DOUBLE( piDstS, rgb_y, rgb_y1, b_u0, g_uv0, r_v0, 255, 255); //0,0
      pbtY += 2;
    }
...
)
{
  ...
  wci_memcpy(pbtDst, pbtY, iSizeY );
  wci_memcpy( &pbtDst[iSizeY], pbtU, iSizeUV);
  wci_memcpy(&pbtDst[iSizeY + iSizeUV], pbtV, iSizeUV);
