...
)
{
  ...
  while( y < rSize.cy )
  {
    ...
    wci_memset( aiMeanRest, 0, DPCM_MAX_X_BLOCKS*sizeof(int) );
...
{
  wci_memset( prState, 0, sizeof(ADPCM_STATE) );
  ...
  wci_memset( prState->pshPrevLine, 0, rBlockSize.cx * sizeof(short) );
...
)
{
  ...
  if( iDecodeLines > 0 )
  {
    ...
    for ( nLines=0; nLines<iDecodeLines; )
    {
      ...
      wci_memcpy( prState->pshPrevLine, pshLL - iStride, prState->rBlockSize.cx*sizeof(short) );
