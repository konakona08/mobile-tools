                        ...
                        short *Tmp_Buffer)
{
  ...
  for(i=0;i<sizey/2;i++)
  {
    ...
    {
      wci_memcpy(p_ll, p_block, GROUP_HALF_SIZE_X*sizeof(short));
      p_ll += GROUP_HALF_SIZE_X;p_block+=GROUP_SIZE_X;
    }
    if(res_in_row_d2)
    {
      wci_memcpy(p_ll, p_block, res_in_row_d2*sizeof(short));
                ...
                short *Tmp_Buffer)
{
  ...
  for(j=0;j<sizey;j++)
  {
     ...
     ptr_ord=Tmp_Buffer;
      ...
      {
        wci_memcpy(ptr_ord,ptr_l,4*sizeof(short));
        ptr_ord+=4;
        wci_memcpy(ptr_ord,ptr_h,4*sizeof(short));
        ptr_ord+=4;
        ptr_l+=4;ptr_h+=4;
      }
      if(rest_x)
      {
        wci_memcpy(ptr_ord,ptr_l,rest_x_half*sizeof(short));
        ptr_ord+=rest_x_half;
        wci_memcpy(ptr_ord,ptr_h,rest_x_half*sizeof(short));
      }
      wci_memcpy(ptr_st,Tmp_Buffer,sizex*sizeof(short));
      ptr_st+=ImSizex;
  }
...
// Horizontal ordering
  ...
  for(j=0;j<sizey;j++)
  {
    ...
    for(i=0;i<num_xgroup;i++)
    {
      wci_memcpy(ptr_l,ptr_ord,4*sizeof(short));
      ptr_ord+=4;ptr_l+=4;
      wci_memcpy(ptr_h,ptr_ord,4*sizeof(short));
      ptr_ord+=4;ptr_h+=4;
    }
    if(rest_x)
    {
      wci_memcpy(ptr_l,ptr_ord,rest_x_half*sizeof(short));
      ptr_ord+=rest_x_half;ptr_l+=rest_x_half;
      wci_memcpy(ptr_h,ptr_ord,rest_x_half*sizeof(short));
    }
    wci_memcpy(ptr_st,Tmp_Buffer,sizex*sizeof(short));
    ptr_st+=ImSizex;
  }
...
// H-transform
  ...
  ptr_orig=Orig+shiftB;
    ...
    // L-corr 
  	...
  	wci_memcpy(ptr_orig-sizex,BufferH,sizex*sizeof(short));
...
static void wci_inv_lift_wav_block_ord(short *Split, int posx, int posy,int sizex, int sizey, int ImSizeX, short *Tmp_Buffer)
{
  ...
  // H-inverse transform	
	...
	for(i=0;i<sizey;i++)
	{
		wci_memcpy(BufferH,ptr_orig,sizex*sizeof(short));
...
void wci_inv_wavlet_insert_ll_c(short *ptr_small, int pos_x, int pos_y, int size_x, int size_y, int ImSizeX, short *ptr_big)
{
  ...
  if(res_in_row_d2 != 0)
  {
    while(dest0<p_fin)
    {
      ...
      {
        wci_memcpy(dest, p_small, GROUP_HALF_SIZE_X*sizeof(short));
        p_small += GROUP_HALF_SIZE_X;
      }
      wci_memcpy(dest, p_small, res_in_row_d2*sizeof(short));
...
void wci_inv_insert_2x2_ll(short *ptr_small, int ImSizeX, short *ptr_big)
{
  ...
  wci_memcpy(dest, p_small, 2*sizeof(short));
  p_small += (ImSizeX/2);
