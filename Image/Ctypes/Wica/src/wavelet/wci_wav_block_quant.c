...
#define CHECK_AND_DO_REC_PSPQ_WS(sum, dz, prQV, cur, bot, ptrDes)	\
  ...
  {																	\
    wci_memset(cur, 0, 2*sizeof(short));	cur+=2;						\
    wci_memset(bot, 0, 2*sizeof(short));	bot+=2;						\
  }																	\
  ptrDes++;
...
#define CHECK_AND_DO_REC(sum, dz, prQV, cur, bot, ptrDes)	\
  ...
  {															                          \
    wci_memset(cur, 0, 2*sizeof(short));						          \
    wci_memset(bot, 0, 2*sizeof(short));						          \
  }															                          \
  ptrDes++;
...
*/
  ...
  wci_memset( pbtPartition, 0, 3*nP_Length);
