...
#include "wavelet/wci_wav_stripe.h"
#include "wavelet/wci_wav_stripe_trans.h"
#include "wavelet/wci_wav_block_quant.h"
#include "wavelet/wci_wav_stripe_quant.h"
#include "wavelet/wci_adpcm.h"
...
)
{
  ...
  wci_memset( decompressor, 0, sizeof(QSP_STRIPE_DECOMPRESSOR) );
...
)
{
  ...
  wci_memset( &rMaxSize, 0, sizeof(SIZE) );
...
//  wci_set_image_size_ex( prFrameHeader->rImageSize, prWavParam->iMaxWavSteps, prDec );
...

  {
    ...
    wci_memset( &prDec->image_stripe, 0, sizeof(DIFF_IMAGE_EX) );
...
)
{
  ...
  wci_memset( prInfo, 0, sizeof(FRAME_IMAGE_INFO) );
...
)
{
  ...
  if(!dec->prMemHeap)
    wci_memset(dec, 0, sizeof(STRIPE_DECODER));
...
)
{
...

  {
    ...
    wci_memcpy( buffer, dec->rSLD.q_size, sizes );
    wci_memcpy( &buffer[sizes], dec->rSLD.s_size, sizes );
...
)
{
  ...
  wci_memset( &param, 0, sizeof(param) );
  param.eColorSpace  = CSP_NULL;
  param.iProcBufSize = wci3_get_decoder_buffer_size(&info, 0);
  param.pbtProcBuf   = (unsigned char *)wci_malloc(param.iProcBufSize);
  param.pbtImage     = (unsigned char *)wci_malloc(4*MAX_IMAGE_WIDTH*MAX_IMAGE_HEIGHT);
  ...
  wci_free( param.pbtImage );
  wci_free( param.pbtProcBuf );
...
)
{
  ...
  if ( wci_strncmp( szPreamble, FRAME_CACHE_PREAMBLE, PREAMBLE_SIZE ) == 0 )
  {
    ...
    wci_memcpy( dec->rSLD.q_size, buffer, sizes * sizeof(int16_t) );
    buffer += sizes * sizeof(int16_t);
    wci_memcpy( dec->rSLD.s_size, buffer, sizes * sizeof(int16_t) );
    buffer += sizes * sizeof(int16_t);
...
)
{
  ...
  wci_memset(prDecoder, 0, sizeof(STRIPE_DECODER));
...
)
{
  ...
  {
    wci_memset( prStripeDec->rQSP.pbtQ, -1, size );
  }
