...
#include "wavelet/wci_wav_layer.h"
#include "wavelet/wci_wav_quant.h"
#include "image/wci_image_convert.h"
#include "main/wci_encoder.h"
...
int wci_encoder_create(ENC_PARAM * pParam)
{
  ...
  if (prEnc == NULL)
  {
	...
	wci_memset(prEnc, 0, sizeof(ENCODER));
...
//    wci_set_image_size( prEnc, prFrame->iWidth, prFrame->iHeight, prEnc->rWavParam.iMaxWavSteps );
  ...
  wci_memset( &rQSP_Size, 0, sizeof(rQSP_Size) );
...
{
  wci_memset( prParam, 0, sizeof(ENC_PARAM) );
...
{
  wci_memset( prFrame, 0, sizeof(ENC_FRAME) );
...
)
{
  ...
  wci_memset( &rInitParam, 0, sizeof(rInitParam) );
  ...
  wci_memset( rEncStat , 0, sizeof(rEncStat ) );
  ...
  if (rEncFrame.iGlobalFlags & USE_FRAME_CACHE)
  {
    ...
    wci_memcpy( &prStream->pbtData[prStream->iOffset], cache_stream.pbtData, cache_stream.iOffset );
