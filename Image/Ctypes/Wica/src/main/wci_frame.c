...
#include "wavelet/wci_wav_codec.h"
...
)
{
  ...
  if ( wci_strncmp( szPreamble, szFramePreamble, PREAMBLE_SIZE ) != 0 )
  {
    return ERR_FORMAT;
  }
...
)
{
  ...
  wci_memset( prInfo, 0, sizeof(STREAM_INFO) );
...
)
{
  ...
  while ( iOffset + MAX_HEADER_SIZE <= prStream->iLength )
  {
    ...
    if ( wci_strncmp( szPreamble, PREAMBLE, PREAMBLE_PREFIX_SIZE ) == 0 )
    {
      ATOM_TYPE type = AT_UNKNOWN;
      
           if( wci_strncmp( szPreamble, FRAME_PREAMBLE      , PREAMBLE_SIZE ) == 0 ) type = AT_FRAME;
      else if( wci_strncmp( szPreamble, STREAM_INFO_PREAMBLE, PREAMBLE_SIZE ) == 0 ) type = AT_STREAM_INFO;
      else if( wci_strncmp( szPreamble, FRAME_CACHE_PREAMBLE, PREAMBLE_SIZE ) == 0 ) type = AT_FRAME_CACHE;
      else if( wci_strncmp( szPreamble, FAT_PREAMBLE        , PREAMBLE_SIZE ) == 0 ) type = AT_FAT;
      else if( wci_strncmp( szPreamble, BOOTANIM_PREAMBLE   , PREAMBLE_SIZE ) == 0 ) type = AT_BOOTANIM;
